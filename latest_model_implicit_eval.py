...
class SingleViewto3D(nn.Module):
    def __init__(self, args):
        ...
        self.encoded_feat_linear = nn.Linear(512*3,256)
    ...
    def forward(self, images, args, indices):
        ...
        if not args.load_feat:
            images_normalize_1 = self.normalize(images_1.permute(0,3,1,2))
            images_normalize_2 = self.normalize(images_2.permute(0,3,1,2))
            images_normalize_3 = self.normalize(images_3.permute(0,3,1,2))
            encoded_feat_1 = self.encoder(images_normalize_1).squeeze(-1).squeeze(-1) # b x 512
            encoded_feat_2 = self.encoder(images_normalize_2).squeeze(-1).squeeze(-1)
            encoded_feat_3 = self.encoder(images_normalize_3).squeeze(-1).squeeze(-1)
        else:
            encoded_feat = images # in case of args.load_feat input images are pretrained resnet18 features of b x 512 size
        encoded_feat_concatenated = torch.concat((encoded_feat_1, encoded_feat_2, encoded_feat_3), dim=-1)
        encoded_feat = self.encoded_feat_linear(encoded_feat_concatenated)
        #if train
        # x = self.coords[indices,:]

        #if eval
        x = self.coords.reshape(64,64,64,3)[indices][:, indices][:, :, indices,:]
        x = x.reshape(32*32*32,3)
