...
def evaluate_iou(metrics ,predictions, voxels_gt, args):
    """
    ...
    occ1 = np.asarray(predictions.cpu().detach())
    occ2 = np.asarray(voxels_gt.cpu().detach())
    if occ1.ndim >= 2:
    ...
    # Compute IOU
    union = (occ1 | occ2).astype(np.float32).sum(axis=-1)
    intersection = (occ1 & occ2).astype(np.float32).sum(axis=-1)
...
def evaluate_model(args):
    ...
    if args.load_checkpoint:
        ...
        checkpoint = torch.load(f'checkpoint_implicit_{args.model_name}_99000.pth')
    ...
    for step in range(start_iter, max_iter):
        ...
        images_gt, mesh_gt, indices, voxgt = preprocess(feed_dict, args)
        ...
        metrics = evaluate(predictions, mesh_gt, thresholds, args, voxgt)
